// =============================================================================
// LRBC — Fix two errors:
//   1. "useSidebar must be used within SidebarProvider" crash
//      (dashboard was missing SidebarProvider wrapper)
//   2. Syntax errors in app/api/site-content/route.ts
//      (unescaped apostrophes in 'individual's' / 'that's' broke the strings —
//       this was the cause of the 500 Internal Server Error)
//
...
// ─────────────────────────────────────────────────────────────────────────────
// FIX 1: dashboard — restore SidebarProvider
// ─────────────────────────────────────────────────────────────────────────────
const dashPath = path.join(root, 'components/dashboar-view/dashboar-view.tsx');
fs.copyFileSync(dashPath, dashPath + '.bak');
          ...
          <div className="rounded-2xl border bg-card shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between border-b bg-background/95 px-4 py-3">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mt-0.5">{currentPage.description}</p>
              </div>
            ...
            <div className="overflow-y-auto max-h-[600px]">
              <div className="space-y-4 p-4">{renderPage()}</div>
            </div>
...
console.log('fixed: components/dashboar-view/dashboar-view.tsx (SidebarProvider restored)');

// ─────────────────────────────────────────────────────────────────────────────
// FIX 2: site-content route — repair broken apostrophe strings
// (only if the file exists — it comes from setup-full-admin.js)
// ─────────────────────────────────────────────────────────────────────────────
const routePath = path.join(root, 'app/api/site-content/route.ts');
if (fs.existsSync(routePath)) {
  let src = fs.readFileSync(routePath, 'utf8');
  fs.copyFileSync(routePath, routePath + '.bak');

  let fixCount = 0;
  const lines = src.split('\n');
  const fixedLines = lines.map((line) => {
    // Match: identifier: 'value...',  where value contains an unescaped '
  ...
  if (fixCount > 0) {
    fs.writeFileSync(routePath, fixedLines.join('\n'), 'utf8');
    console.log('fixed: app/api/site-content/route.ts (' + fixCount + ' broken strings repaired)');
  } else {
    console.log('ok: app/api/site-content/route.ts (no broken strings found)');
  }
} else {
  console.log('skip: app/api/site-content/route.ts not present');
}

console.log('\nDone. If the page is still erroring, clear the cache:');
